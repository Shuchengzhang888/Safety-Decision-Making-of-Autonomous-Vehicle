...
class Evaluation(object):
    ...
    def train(self):
        ...
        #     self.run_batched_episodes()
        # else:
        #     self.run_episodes(record = False)
        self.close()
    ...
    def test(self):
        ...
        try:
            ...
            pass
        self.run_episodes(record = True)
        self.close()

    def run_episodes(self, record):
        ...
        for self.episode in range(self.num_episodes):
            ...
            rewards = []
            speeds = []
            # ini_position = self.env.unwrapped.vehicle.position[0]
            ...
            ##
            # print(speeds)
            total_sum_rewards.append(np.sum(rewards))
            total_avg_speeds.append(np.mean(speeds))
            total_angles.append(self.env.unwrapped.vehicle.heading)
            print(self.env.unwrapped.vehicle.heading)
            # total_dis.append(self.env.unwrapped.vehicle.position[0] - ini_position)
            if info['crashed']:
                crash_times +=1
            ##
            ...
            # total_dis.append(duration*total_avg_speeds[-1])
            self.after_all_episodes(self.episode, rewards, duration)
            self.after_some_episodes(self.episode, rewards)

        if record:
            import pandas as pd
            # dict = {'total_sum_rewards': total_sum_rewards,'total_avg_speeds': total_avg_speeds,'total_dis': total_dis, 'crash_times': crash_times}
            dict = {'total_sum_rewards': total_sum_rewards,'total_avg_speeds': total_avg_speeds,'total_angles': total_angles}
            df = pd.DataFrame(dict)
            df.to_csv('rt_lstm_att_300.csv')
            print(np.mean(total_sum_rewards), np.mean(total_avg_speeds))
    ...
    def run_batched_episodes(self):
        ...
        """
        episode = 0
        episode_duration = 60  # TODO: use a fixed number of samples instead
        ...
        for batch, batch_size in enumerate(batch_sizes):
            ...
            base_seed = self.env.seed(batch * cpu_processes)[0]
