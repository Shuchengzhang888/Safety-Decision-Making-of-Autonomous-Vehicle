...
###############################################################################################################
class Kinematic_Observation_Simple(ObservationType):
    ...
    def observe(self) -> np.ndarray:
        ...
        self.observer_vehicle.close_vehicles = close_vehicles

        origin = self.observer_vehicle
        left_lane_available = 1
        right_lane_available = 1
        if origin.lane_index[2] == 0: 
            left_lane_available = 0
        elif origin.lane_index[2] == 3:
            right_lane_available = 0
        else:
            pass
        vx_0 = origin.velocity[0]*np.cos(origin.heading) + origin.velocity[1]*np.sin(origin.heading)
        vy_0 = origin.velocity[1]*np.cos(origin.heading) - origin.velocity[0]*np.sin(origin.heading)
        obss = np.array([1, left_lane_available, right_lane_available, vx_0, vy_0])

        if close_vehicles:
            for v in close_vehicles:
                x = (v.position[0]-origin.position[0])*np.cos(origin.heading) + (v.position[1]-origin.position[1])*np.sin(origin.heading)
                y = (v.position[1]-origin.position[1])*np.cos(origin.heading) - (v.position[0]-origin.position[0])*np.sin(origin.heading)
                vx = (v.velocity[0]-origin.velocity[0])*np.cos(origin.heading) + (v.velocity[1]-origin.velocity[1])*np.sin(origin.heading)
                vy = (v.velocity[1]-origin.velocity[1])*np.cos(origin.heading) - (v.velocity[0]-origin.velocity[0])*np.sin(origin.heading)
                ...
                elif self.input_size == 25:
                    #5 + 4*5
                    obss = np.hstack((obss,[1, x, y, vx, vy]))
...
class Kinematic_Observation_Traj3D_RT(ObservationType):
    ...
    def space(self) -> spaces.Space:
        return spaces.Box(shape=(self.traj_range, self.vehicles_count, len(self.features)), low=-np.inf, high=np.inf, dtype=np.float32)
    ...
    def observe(self) -> np.ndarray:
        ...
        for t in range(self.traj_range):
            obs[t][0][0] = 1
            obs[t][0][1] = np.clip(utils.lmap(self.observer_vehicle.trajectory[t].position[0], self.features_range["x"], [-1, 1]), -1, 1)
            obs[t][0][2] = np.clip(utils.lmap(self.observer_vehicle.trajectory[t].position[1], self.features_range["y"], [-1, 1]), -1, 1)
            obs[t][0][3] = np.clip(utils.lmap(self.observer_vehicle.trajectory[t].velocity[0], self.features_range["vx"], [-1, 1]), -1, 1)
            obs[t][0][4] = np.clip(utils.lmap(self.observer_vehicle.trajectory[t].velocity[1], self.features_range["vy"], [-1, 1]), -1, 1)
            obs[t][0][5] = self.observer_vehicle.trajectory[t].angle_offset
            obs[t][0][6] = self.observer_vehicle.trajectory[t].index_lane[2]/4
        ...
        self.observer_vehicle.close_vehicles = close_vehicles
        if close_vehicles:
            for i, v in enumerate(close_vehicles):
                for t in range(self.traj_range):
                    obs[t][i+1][0] = 1

                    trans_matrix = np.linalg.inv(np.array([[np.cos(self.observer_vehicle.heading), np.sin(-self.observer_vehicle.heading), self.observer_vehicle.position[0]],
                                                                [np.sin(self.observer_vehicle.heading), np.cos(self.observer_vehicle.heading), self.observer_vehicle.position[1]],
                                                                [0, 0, 1]]))
                    new_coor = trans_matrix@np.array([v.trajectory[t].position[0], v.trajectory[t].position[1], 1])
                    #print(new_coor, trans_matrix, np.array([v.trajectory[t].position[0], v.trajectory[t].position[1], 1]))
                    velo_trans_matrix = np.linalg.inv(np.array([[np.cos(self.observer_vehicle.heading), np.sin(-self.observer_vehicle.heading), self.observer_vehicle.velocity[0]],
                                                                [np.sin(self.observer_vehicle.heading), np.cos(self.observer_vehicle.heading), self.observer_vehicle.velocity[1]],
                                                                [0, 0, 1]]))
                    new_velo = velo_trans_matrix@np.array([v.trajectory[t].velocity[0], v.trajectory[t].velocity[1], 1])
                    #print(new_velo, velo_trans_matrix, np.array([v.trajectory[t].velocity[0], v.trajectory[t].velocity[1], 1]))
                     
                    obs[t][i+1][1] = np.clip(utils.lmap(new_coor[0], [-100, 100], [-1, 1]), -1, 1)
                    obs[t][i+1][2] = np.clip(utils.lmap(new_coor[1], [-100, 100], [-1, 1]), -1, 1)
                    obs[t][i+1][3] = np.clip(utils.lmap(new_velo[0], self.features_range["vy"], [-1, 1]), -1, 1)
                    obs[t][i+1][4] = np.clip(utils.lmap(new_velo[1], self.features_range["vx"], [-1, 1]), -1, 1)
                    obs[t][i+1][5] = v.trajectory[t].angle_offset
                    obs[t][i+1][6] = v.trajectory[t].index_lane[2]/4

        return obs
class Kinematic_Observation_Traj3D_C(ObservationType):
    FEATURES: List[str] = ['presence', 'x', 'y', 'vx', 'vy','cos_h', 'ang_off']
    def __init__(self, env: 'AbstractEnv',
                 features: List[str] = None,
                 vehicles_count: int = 5,
                 features_range: Dict[str, List[float]] = None,
    ...
    def observe(self) -> np.ndarray:
        ...
        return obs
class Kinematic_Observation_Traj3D_HW(ObservationType):
    FEATURES: List[str] = ['presence', 'x', 'y', 'vx', 'vy','cos_h', 'sin_h']
    def __init__(self, env: 'AbstractEnv',
                 features: List[str] = None,
                 vehicles_count: int = 5,
    ...
    def observe(self) -> np.ndarray:
        ...
        for t in range(self.traj_range):
            obs[t][0][0] = 1
            obs[t][0][1] = np.clip(utils.lmap(self.observer_vehicle.trajectory[t].position[0]-self.observer_vehicle.position[0], self.features_range["x"], [-1, 1]), -1, 1)
            obs[t][0][2] = np.clip(utils.lmap(self.observer_vehicle.trajectory[t].position[1]-self.observer_vehicle.position[1], self.features_range["y"], [-1, 1]), -1, 1)
            obs[t][0][3] = np.clip(utils.lmap(self.observer_vehicle.trajectory[t].velocity[0]-self.observer_vehicle.velocity[0], self.features_range["vx"], [-1, 1]), -1, 1)
            obs[t][0][4] = np.clip(utils.lmap(self.observer_vehicle.trajectory[t].velocity[1]-self.observer_vehicle.velocity[1], self.features_range["vy"], [-1, 1]), -1, 1)
            obs[t][0][5] = np.cos(self.observer_vehicle.trajectory[t].heading)
            obs[t][0][6] = self.observer_vehicle.trajectory[t].index_lane[2]
        # Add nearby traffic
        close_vehicles = self.env.road.close_vehicles_to(self.observer_vehicle,
                                                         self.env.PERCEPTION_DISTANCE,
                                                         count=self.vehicles_count - 1,
        ...
        if close_vehicles:
            for i, v in enumerate(close_vehicles):
                for t in range(self.traj_range):
                    obs[t][i+1][0] = 1
                    obs[t][i+1][1] = np.clip(utils.lmap(v.trajectory[t].position[0]-self.observer_vehicle.position[0], self.features_range["x"], [-1, 1]), -1, 1)
                    obs[t][i+1][2] = np.clip(utils.lmap(v.trajectory[t].position[1]-self.observer_vehicle.position[1], self.features_range["y"], [-1, 1]), -1, 1)
                    obs[t][i+1][3] = np.clip(utils.lmap(v.trajectory[t].velocity[0]-self.observer_vehicle.velocity[0], self.features_range["vy"], [-1, 1]), -1, 1)
                    obs[t][i+1][4] = np.clip(utils.lmap(v.trajectory[t].velocity[1]-self.observer_vehicle.velocity[1], self.features_range["vx"], [-1, 1]), -1, 1)
                    obs[t][i+1][5] = np.cos(v.trajectory[t].heading)
                    obs[t][i+1][6] = v.trajectory[t].index_lane[2]

        return obs

class Kinematic_Observation_Traj_RT(ObservationType):
    FEATURES: List[str] = ['presence', 'x', 'y', 'vx', 'vy','cos_h', 'sin_h', 'cos_d', 'sin_d']
    def __init__(self, env: 'AbstractEnv',
                 features: List[str] = None,
                 vehicles_count: int = 5,
                 features_range: Dict[str, List[float]] = None,
    ...
    def observe(self) -> np.ndarray:
        ...
        #################################################
        # replace sin_h with lane num, cos_d with pred_x, sin_d with pred_y
        obs[0][6] = self.observer_vehicle.lane_index[2]/4
        obs[0][7] = np.clip(utils.lmap(self.observer_vehicle.pred_position1[0], [0, 140], [-1, 1]), -1, 1)
        obs[0][8] = np.clip(utils.lmap(self.observer_vehicle.pred_position1[1], [-50, 0], [-1, 1]), -1, 1)
        for i, v in enumerate(close_vehicles):
            obs[i+1][6] = v.lane_index[2]/4
            obs[i+1][7] = np.clip(utils.lmap(v.pred_position1[0], [0, 140], [-1, 1]), -1, 1)
            obs[i+1][8] = np.clip(utils.lmap(v.pred_position1[1], [-50, 0], [-1, 1]), -1, 1)
        #################################################

        return obs
class Kinematic_Observation_Traj_C(ObservationType):
    FEATURES: List[str] = ['presence', 'x', 'y', 'vx', 'vy','ang_off', 'sin_h', 'cos_d', 'sin_d']
    def __init__(self, env: 'AbstractEnv',
                 features: List[str] = None,
                 vehicles_count: int = 5,
                 features_range: Dict[str, List[float]] = None,
    ...
    def observe(self) -> np.ndarray:
        ...
        # replace sin_h with lane num, cos_d with pred_x, sin_d with pred_y
        obs[0][6] = self.observer_vehicle.lane_index[2]/4
        obs[0][7] = np.clip(utils.lmap(self.observer_vehicle.pred_position1[0], [-40, 140], [-1, 1]), -1, 1)
        obs[0][8] = np.clip(utils.lmap(self.observer_vehicle.pred_position1[1], [-160, 20], [-1, 1]), -1, 1)
        for i, v in enumerate(close_vehicles):
            obs[i+1][6] = v.lane_index[2]/4
            obs[i+1][7] = np.clip(utils.lmap(v.pred_position1[0], [-40, 140], [-1, 1]), -1, 1)
            obs[i+1][8] = np.clip(utils.lmap(v.pred_position1[1], [-160, 20], [-1, 1]), -1, 1)
        #################################################

        return obs
class Kinematic_Observation_Traj_HW(ObservationType):
    FEATURES: List[str] = ['presence', 'x', 'y', 'vx', 'vy','cos_h', 'sin_h', 'cos_d', 'sin_d']
    def __init__(self, env: 'AbstractEnv',
                 features: List[str] = None,
                 vehicles_count: int = 5,
    ...
    def observe(self) -> np.ndarray:
        ...
        #################################################
        # replace sin_h with lane num, cos_d with pred_x, sin_d with pred_y
        obs[0][6] = self.observer_vehicle.lane_index[2]
        obs[0][7] = 1
        obs[0][8] = 1
        for i, v in enumerate(close_vehicles):
            obs[i+1][6] = v.lane_index[2]
            side_lanes = self.env.road.network.all_side_lanes(self.observer_vehicle.lane_index)
            x_norm = utils.lmap(v.pred_position1[0]-self.observer_vehicle.pred_position1[0], [-5.0 * Vehicle.MAX_SPEED, 5.0 * Vehicle.MAX_SPEED], [-1, 1])
            y_norm = utils.lmap(v.pred_position1[1]-self.observer_vehicle.pred_position1[1], [-AbstractLane.DEFAULT_WIDTH * len(side_lanes), AbstractLane.DEFAULT_WIDTH * len(side_lanes)], [-1, 1])
            x_clip = np.clip(x_norm, -1, 1)
            y_clip = np.clip(y_norm, -1, 1)
            obs[i+1][7] = x_clip
            obs[i+1][8] = y_clip
        # replace self x,y
        obs [0][1] = 1
...
class Kinematic_Observation_Traj_Nopre(ObservationType):
    FEATURES: List[str] = ['presence', 'x', 'y', 'vx', 'vy','cos_h', 'sin_h']
    def __init__(self, env: 'AbstractEnv',
                 features: List[str] = None,
                 vehicles_count: int = 5,
...
def observation_factory(env: 'AbstractEnv', config: dict) -> ObservationType:
    ...
    elif config["type"] == "Kinematic_Observation_Simple":
        return Kinematic_Observation_Simple(env, **config)
    elif config["type"] == "Kinematic_Observation_Traj3D_RT":
        return Kinematic_Observation_Traj3D_RT(env, **config)
    elif config["type"] == "Kinematic_Observation_Traj3D_HW":
        return Kinematic_Observation_Traj3D_HW(env, **config)
    elif config["type"] == "Kinematic_Observation_Traj3D_C":
        return Kinematic_Observation_Traj3D_C(env, **config)
    elif config["type"] == "Kinematic_Observation_Traj_RT":
        return Kinematic_Observation_Traj_RT(env, **config)
    elif config["type"] == "Kinematic_Observation_Traj_HW":
        return Kinematic_Observation_Traj_HW(env, **config)
    elif config["type"] == "Kinematic_Observation_Traj_C":
        return Kinematic_Observation_Traj_C(env, **config)
    elif config["type"] == "Kinematic_Observation_Traj_Nopre":
        return Kinematic_Observation_Traj_Nopre(env, **config)
